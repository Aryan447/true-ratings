...
export default function Home() {
  ...
  const getSeasonBest = (episodes: Episode[]): BestWorst | null => {
  ...
  const getSeasonWorst = (episodes: Episode[]): BestWorst | null => {
                ...
                <div key={season} className="bg-gray-900 p-4 rounded-lg">
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2">
                    {episodes.map((ep, i) => {
                      ...
                      const isGlobalWorst = globalWorst?.ep.imdbID === ep.imdbID;

                      return (
                        <div
                          key={i}
                          className={`p-2 rounded text-center font-medium text-sm relative ${
                            isNaN(rating) ? "bg-gray-700" : getColor(rating)
                          } ${isSeasonBest || isGlobalBest || isSeasonWorst || isGlobalWorst ? "ring-2 ring-white" : ""}`}
                        >
                          {isSeasonBest && !isGlobalBest && (
                            <div className="text-[12px] leading-tight mb-1 text-green-300">
                              Highest Rated
                            </div>
                          )}
                          {isGlobalBest && (
                          ...
                          {isSeasonWorst && !isGlobalWorst && (
                            <div className="text-[12px] leading-tight mb-1 text-red-300">
                              Lowest Rated
                            </div>
                          )}
                          {isGlobalWorst && (
